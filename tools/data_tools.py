...
def checkin_data(args: dict, cfg, state) -> dict:
    """Check in current training data to git — durable backup before modifications."""
    try:
        version = args.get("version", f"v{state.model_version}")
        desc = args.get("description", "")

        dest = _PROJECT_ROOT / "data" / "checked_in" / version
        ...
        from collections import Counter
        counts = Counter()
        train_src = cfg.data_dir / "train.jsonl"
        if train_src.exists():
            for line in train_src.read_text().splitlines():
                line = line.strip()
                if line:
                    try:
        ...
        meta = {
            ...
            "avg_score": round(state.avg_score, 4) if state.scores else None,
        }
        ...
        if proc.returncode != 0:
            log_print(f"  [checkin_data] git add failed: {proc.stderr}")

        commit_msg = f"Check in {version} training data ({sum(counts.values())} examples)"
        if desc:
            commit_msg += f" — {desc}"
        proc = subprocess.run(
        ...
        if proc.returncode != 0 and "nothing to commit" not in proc.stderr + proc.stdout:
            log_print(f"  [checkin_data] git commit failed: {proc.stderr}")
        ...
        if proc.returncode != 0:
            log_print(f"  [checkin_data] git push failed: {proc.stderr}")

        log_print(f"  [checkin_data] {version}: {sum(counts.values())} examples -> {dest}")
        log_print(f"  [checkin_data] Committed and pushed to git")
